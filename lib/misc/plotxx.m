function [ax,hl1,hl2] = plotxx(x1,y1,x2,y2,xlabels,ylabels,col1,col2);
...
FSz=17;
hl1=line(x1,y1,'Color',col1, 'LineWidth', 2);
ax(1)=gca;
set(ax(1),'Position',[0.12 0.12 0.75 0.70])
set(ax(1),'XColor',col1,'YColor',col1,'FontSize', FSz  );

ax(2)=axes('Position',get(ax(1),'Position'),...
   'XAxisLocation','top',...
   'YAxisLocation','right',...
   'Color','none',...
   'XColor',col2,'YColor',col2,...
   'FontSize', FSz );

set(ax,'box','off')

hl2=line(x2,y2,'Color',col2,'Parent',ax(2), 'LineWidth', 2);

%label the two x-axes

set(get(ax(1),'xlabel'),'string',xlabels{1}, 'FontSize',FSz )
set(get(ax(2),'xlabel'),'string',xlabels{2}, 'FontSize',FSz)
set(get(ax(1),'ylabel'),'string',ylabels{1}, 'FontSize',FSz)
set(get(ax(2),'ylabel'),'string',ylabels{2}, 'FontSize',FSz)