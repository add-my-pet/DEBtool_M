...
% temporary
mat_ind = {'b'; 'j'; 'p'};
mat_label = {'at birth'; 'at metamorphosis'; 'at puberty'};
...
pars_tj  = [g; k; l_T; v_Hb; v_Hj; v_Hp]; % get_tj
pars_R   = [kap; kap_R; g; k_J; k_M; L_T; v; U_Hb; U_Hj; U_Hp];  % reprod_rate_j
pars_tm  = [g; k; l_T; v_Hb; v_Hp; h_a/ (p_M/ E_G)^2; s_G];     % get_tm_s
pars_pow = [kap; kap_R; g; k_J; k_M; L_T; v; U_Hb; U_Hj; U_Hp]; % scaled_power_j
...
[t_j, t_p, t_b, l_j, l_p, l_b, l_i, rho_j, rho_B, info] = get_tj(pars_tj, f, L_b/ L_m); 
if info ~= 1
...
xi_W_E   = (mu_V + mu_E * m_Em * f)/ (f * w);  % kJ/g, whole-body energy density (no reprod buffer), <E + E_V>
...
for i = 1:length(mat_ind)
  if mat_ind{i} ~= 'b'
    stri = mat_ind{i};
%     labeli = mat_label{i};
    eval(['pACSJGRD_', stri, ' = p_ref * scaled_power_j(L_', stri, ', f, pars_pow, l_b, l_j, l_p);']);  % assimilation, dissipation, growth power
  end
end
