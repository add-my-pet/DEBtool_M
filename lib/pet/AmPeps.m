...
AmPgui;
if isempty(infoAmPgui) | infoAmPgui == 0 
  return % stay in gui  
end

% finish AmPgui and proceed with AmPeps
close(hclimateLand,hclimateSea,hecozones,hoceans);
path = 'https://www.bio.vu.nl/thb/deb/deblab/add_my_pet/entries/';

if infoAmPgui == 2 % skip the rest of AmPeps and proceed to copy 4 AmP source files of AmP species and edit in Matlab
  if ismac
    ...
    eval(['!powershell wget ', path, 'mydata_', metaData.species, '.m',  ' -O ', 'mydata_', metaData.species, '.m'])
    eval(['!powershell wget ', path, 'pars_init_', metaData.species, '.m',  ' -O ', 'pars_init_', metaData.species, '.m'])
    eval(['!powershell wget ', path, 'predict_', metaData.species, '.m',  ' -O ', 'predict_', metaData.species, '.m'])
    eval(['!powershell wget ', path, 'run_', metaData.species, '.m',  ' -O ', 'run_', metaData.species, '.m'])
  end
...
% proceed to writing 4 AmP source files for new species for AmP
