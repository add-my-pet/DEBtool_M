...
function sol = postProcess_mmea(results_mmea)
  ...
  if ismac || isunix % save a copy; will be restore at the end
    ...
    system(['powershell cp results_',my_pet,'.mat results_',my_pet,'mmea_copy.mat']);
  end
  ...
  for h = 1:n_rnd % start selection & refinement rounds
    ...
    for i=1:n_sol
      ...
      save(['results_',my_pet,'_mmea_postProcess.mat'], 'par');
      estim_pars;
  ...
  if ismac || isunix
    system(['cp results_',my_pet,'mmea_copy.mat results_',my_pet,'.mat']);
  else
    system(['powershell cp results_',my_pet,'mmea_copy.mat results_',my_pet,'.mat']);
  end
  delete(['results_',my_pet,'mmea_copy.mat'])
