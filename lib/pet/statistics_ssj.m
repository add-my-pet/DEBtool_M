...
function [stat, txt_stat] = statistics_ssj(par, T, f, model) %% T_ref from metapar or input like T?
% created 2015/07/03 by Starrlight Augustine, modified 2015/08/25 by
% starrlight

%% WARNING: work in progress at the moment this makes statistics for std model not ssj !!!
...
model = 'std';
[stat, txt_stat] = statistics_std(par, T, f, model);



% cp = parscomp_st(par);
% v2struct(cp); v2struct(par);  
% 
