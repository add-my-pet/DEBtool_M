...
data.KV  = 6e8;   units.KV  = 'mum^3/dm^3'; label.KV  = 'half saturation coefficient';  bibkey.KV  = 'RicoBern2010';   
...
% uni-variate data
 ...
 %   85 mum3; 5e-11 g/cell GonzQuea2011 
...
%   Isochrysis affinis galbana (45 mum3; 20e-12 g/cell)
%   L < 110 mum: Chaetoceros calcitrans forma pumilus (30-45 mum3; 17.3e-12 g dry/cell RobeChret2004)
%    or 
%   L > 110 mum: Chaetoceros gracilis (80 mum3; 70e-12 g dry/cell)
% temp from 0 till 2 d of age: 25 C
% RicoBern2010 found K = 600 mum3/mul = 13e6 cells/dm3 for larvae
%   K = {p_Am} / kap_X/ E_X/ {F_m} = 14.38e6/dm3, which is in perfect correspondance

%%%% different temperature and constant X = 1400 mum3/mul, f = 0.84 using K = 266 mum3/mul, 
% time is since fertilisation
% age (d) length (mum) at 17 C
data.tL_T17 = [ ...
2	79.06;
5	86.01;
7	91.95;
...
13	271.11;
15	287.00];
units.tL_T32  = {'d', 'mum'};  label.tL_T32 = {'age', 'length'};  
...
% age (d) length (mum) at f = 0.01, X = 70 mum3/mul = 70e6 mum3/dm^3 
data.tL_f010 = [...
1.98	76.95;
5.02	86.69;
6.93	85.71;
...
15.22	263.73;
21.05	283.49];
units.tL_f960  = {'d', 'mum'};  label.tL_f960 = {'age', 'length'};  
...
%   - origin of oysters: hatchery produce (Ifremer La Tremblade, France) from 30 wild broodstock (Maren-Oleron Bay, France); spat reared at Ifremer station (in Bouin, France)
%   - 1-year old oysters conditioned at Ifremer Argenton (France) from February 2002 to February 2003
%   - rearing system: 600l raceways, 20mum filtered running seawater
...
%   - larvae were reared in 4 tanks (300L, 1mum filtered seawater, 23 C, 28 PSU)
%   - food: Isochrysis galbana + Extubocellulus criberiger; concentration: 60 cells/muL (excess to avoid competition)
%   - larvea concentration: day 1 = 13 larvae/mL and progrssively reduced to 1 larvae/mL at day 17 (any selective sieving)
%   - sieving groups: every 48h sieving of the whole population; when length pediveliger larvae > 200mum => tranfered for settlement procedure => 4 sieving groups (SG1 to SG4)
%   - settlement procedure: PVC collector system in raceway => 10mum filtered seawater at 26 C
...
15.2674  229.1339;
17.3096  273.2283];
units.tL_CollBoud1999  = {'d', 'mum'};  label.tL_CollBoud1999 = {'age', 'length'};  
...
%   - intial conditions: L = 95+/-1mum to 331+/-13 mum (D-shaped veligers)
...
%   - Units: respiratory rate (JO) => nL of O_2/h/larvea; L => mum; Wd => mug
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% [Length, mum, JO, nL of O_2/h/larvea, Wd, mg]
% comment Bas: mg must be mug
LJOW_GoulWolo2004 = [...
...
units.LWd_GoulWolo2004  = {'mum', 'mug'};  label.LWd_GoulWolo2004 = {'length', 'dry weight'};  
bibkey.LWd_GoulWolo2004 = 'GoulWolo2004';
%
data.LJO_GoulWolo2004 = LJOW_GoulWolo2004(:,[1 2]);
units.LJO_GoulWolo2004  = {'mum', 'nL O_2/h'};  label.LJO_GoulWolo2004 = {'length', 'O_2 consumption'};  
...
%% Group plots
set1 = {'tL2','tL3','tL4','tL5','tL6','tL7'};       subtitle1 = {'Emme2012 data from bassin 2,3,4,5,6,7'};
set2 = {'tWw2','tWw3','tWw4','tWw5','tWw6','tWw7'}; subtitle2 = {'Emme2012 data from bassin 2,3,4,5,6,7'};
set3 = {'tWd2','tWd3','tWd4','tWd5','tWd6','tWd7'}; subtitle3 = {'Emme2012 data from bassin 2,3,4,5,6,7'};
set4 = {'tJX2','tJX3','tJX4','tJX5','tJX6','tJX7'}; subtitle4 = {'Emme2012 data from bassin 2,3,4,5,6,7'};
set5 = {'tF2','tF3','tF4','tF5','tF6','tF7'}; subtitle5 = {'Emme2012 data from bassin 2,3,4,5,6,7'};
set6 = {'tL_T32','tL_T27','tL_T25','tL_CollBoud1999','tL_T22','tL_T17'}; subtitle6 = {'RicoPouv2009 data at 32, 27, 25, 23, 22, 17 C'};
set7 = {'tL_f960','tL_f920','tL_f900','tL_f730','tL_f710','tL_f360','tL_f017','tL_f010'}; subtitle7 = {'Data at scaled func. resp. 0.96, 0.92, 0.90, 0.73, 0.71, 0.36, 0.017, 0.01'};
set8 = {'tL_T28_Mark2011','tL_T25_Mark2011','tL_T22_Mark2011','tL_T18_Mark2011'}; subtitle8 = {'Mark2011 data at 28, 25, 22, 18 C'};
set9 = {'tWw_T28_Mark2011','tWw_T25_Mark2011','tWw_T22_Mark2011','tWw_T18_Mark2011'}; subtitle9 = {'Mark2011 data at 28, 25, 22, 18 C'};
...
%% Links
metaData.links.id_CoL = '3XGCW'; % Cat of Life
metaData.links.id_ITIS = ''; % ITIS
metaData.links.id_EoL = '46467202'; % Ency of Life
metaData.links.id_Wiki = 'Magallana_gigas'; % Wikipedia
metaData.links.id_ADW = ''; % ADW
metaData.links.id_Taxo = '5488085'; % Taxonomicon
metaData.links.id_WoRMS = '836033'; % WoRMS
metaData.links.id_molluscabase = '836033'; % molluscabase
