function [GlobalParams,GlobalMin, coyotes, costs] = coa(nfevalMAX,n_packs,n_coy)
%% ------------------------------------------------------------------------
...
my_pet = pets{1}; % required for running nm
...
n_par = sum(cell2mat(struct2cell(par.free))); % Get number of parameters
...
lu = get_bounds(qvec(index)', 0.5, 'mult');
D           = size(lu,2);
VarMin      = lu(1,:);
VarMax      = lu(2,:);
...
for c=1:pop_total
   for k=1:n_par; par.(parNm{k}) = coyotes(c,k); end % overwrite free pars
   try
      costs(c,1) = lossFn(func, par, data, auxData, weights);
   catch
      costs(c,1) = pen_val;
   end
end
...
while nfeval<nfevalMAX % Stopping criteria
    ...
    for p=1:n_packs
        ...
        for c=1:n_coy_aux
            ...
            for k=1:n_par; par.(parNm{k}) = new_coyotes(c,k); end % overwrite free pars
            try
               new_cost = lossFn(func, par, data, auxData, weights);
            catch
               costs(c,1) = pen_val;
            end
            nfeval   = nfeval+1;
        ...
        for k=1:n_par; par.(parNm{k}) = new_coyotes(1,k); end % overwrite free pars
        try
         pup_cost    = lossFn(func, par, data, auxData, weights);
        catch
         costs(c,1) = pen_val;
        end
        nfeval      = nfeval + 1;
