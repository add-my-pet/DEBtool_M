...
%   prdData.am = aT_m;
  ...
  Linit = L0.tN0 * del_M; % initial structural length   
  UH0 = maturity(Linit, f, [kap; kap_R; g; k_J; k_M; 0; v; E_Hb/ p_Am; E_Hp/ p_Am]); % initial scaled maturity
  UT_H0 = UH0/ TC_tN;
  % initial
  ...
  U_Einit = f_tN * E_m * Linit^3/ p_Am;
  UT_Einit = U_Einit/ TC_tN;
  X0 = [0;UT_H0; Linit; UT_Einit; 0]; % initial conditions

  [t, Xt] = ode23(@dharep, tN0(:,1), X0,[],par, cPar, tox.tN0, f_tN, UT_E0, TC_tN); % integrate changes in state
  EN0 = Xt(:,1); % select cumulated number of offspring
  
  [t, Xt] = ode23(@dharep, tN1(:,1), X0,[],par, cPar, tox.tN1, f_tN, UT_E0, TC_tN); % integrate changes in state
  EN1 = Xt(:,1); % select cumulated number of offspring
  
  [t, Xt] = ode23(@dharep, tN2(:,1), X0,[],par, cPar, tox.tN2, f_tN, UT_E0, TC_tN); % integrate changes in state
  EN2 = Xt(:,1); % select cumulated number of offspring
  
  [t, Xt] = ode23(@dharep, tN3(:,1), X0,[],par, cPar, tox.tN3, f_tN, UT_E0, TC_tN); % integrate changes in state
  EN3 = Xt(:,1); % select cumulated number of offspring
  
  [t, Xt] = ode23(@dharep, tN4(:,1), X0,[],par, cPar, tox.tN4, f_tN, UT_E0, TC_tN); % integrate changes in state
  EN4 = Xt(:,1); % select cumulated number of offspring

  [t, Xt] = ode23(@dharep, tN5(:,1), X0,[],par, cPar, tox.tN5, f_tN, UT_E0, TC_tN); % integrate changes in state
  ...
  function dX = dharep(t, X, p, c, C, f, UT_0, TC)
...
%   N = X(1);        % cumulative number of offspring
  ...
  kT_M = c.k_M * TC;  kT_e = p.k_e * TC; kT_J = p.k_J * TC; pT_Am = c.p_Am * TC; vT = p.v * TC;
  
  E = U * pT_Am;        % scaled reserve density e = m_E/m_Em (dim-less)
  ...
  Hp = p.E_Hp/ pT_Am;
  R = exp(-s) .* ((1 - p.kap) * SC - kT_J * Hp) * p.kap_R/ UT_0; % reprod rate in %/d
