...
% feeding
J_E1Am_X1 = p.y_E1X1 * JT_X1Am; J_E1Am_X2 = p.y_E1X2 * JT_X2Am; % mol/d.cm^2, max spec assim rate for reserve 1 
J_E2Am_X1 = p.y_E2X1 * JT_X1Am; J_E2Am_X2 = p.y_E2X2 * JT_X2Am; % mol/d.cm^2, max spec assim rate for reserve 2
m_E1m = max(J_E1Am_X1/ vT/ p.MV, J_E1Am_X2/ vT/ p.MV);          % mol/mol, max reserve 1 density
m_E2m = max(J_E2Am_X1/ vT/ p.MV, J_E2Am_X2/ vT/ p.MV);          % mol/mol, max reserve 2 density
...
h_X1Am = JT_X1Am/ p.M_X1; h_X2Am = JT_X2Am/ p.M_X2;             % #/d.cm^2, max spec feeding rates
alpha_X1 = h_X1Am + FT_X1m * X1 + FT_X3m * rho_X2X1 * X2; 
alpha_X2 = h_X2Am + FT_X3m * X2 + FT_X1m * rho_X1X2 * X1;
beta_X1 = FT_X1m * X1 * (1 - rho_X1X2);  beta_X2 = FT_X3m * X2 * (1 - rho_X2X1);
f1 = (alpha_X2 * FT_X1m * X1 - beta_X1 * FT_X3m * X2)/ (alpha_X1 * alpha_X2 - beta_X1 * beta_X2);
f2 = (alpha_X1 * FT_X3m * X2 - beta_X2 * FT_X1m * X1)/ (alpha_X1 * alpha_X2 - beta_X1 * beta_X2);
dcM_X1 = f1 * JT_X1Am * L^2; dcM_X2 = f2 * JT_X2Am * L^2; % mol/d, feeding rates

% assimilation
J_E1A = f1 * p.y_E1X1 * JT_X1Am + f2 * p.y_E1X2 * JT_X2Am; % mol/d.cm^2, {J_E1A}, specific assimilation flux
J_E2A = f1 * p.y_E2X1 * JT_X1Am + f2 * p.y_E2X2 * JT_X2Am; % mol/d.cm^2, {J_E2A}, specific assimilation flux
...
[r j_E1_S j_E2_S j_E1C j_E2C info] = ...         % 1/d, specific growth rate, ....
    sgr_iso_21(m_E1, m_E2, jT_E1S, jT_E2S, p.y_VE1, p.y_VE2, mu_EV, k_E, p.kap, p.rho1); % use continuation
if info == 0 % try to repair in case of lack of convergence by starting from r_0 = 1e-4
  [r j_E1_S j_E2_S j_E1C j_E2C info] = ...       % 1/d, specific growth rate, ....
    sgr_iso_21(m_E1, m_E2, jT_E1S, jT_E2S, p.y_VE1, p.y_VE2, p.mu_EV, k_E, p.kap, rho1, 1e-4);
  if info == 1
...
j_E1P = p.kap * j_E1C - j_E1_S - (r + j_V_S)/ p.y_VE1; % mol/d.mol, rejected flux
j_E2P = p.kap * j_E2C - j_E2_S - (r + j_V_S)/ p.y_VE2; % mol/d.mol
dm_E1 = j_E1A - j_E1C + p.kap_E1 * j_E1P - r * m_E1; % mol/d.mol, change in reserve density
dm_E2 = j_E2A - j_E2C + p.kap_E2 * j_E2P - r * m_E2; % mol/d.mol
...
p_C = p.mu_E1 * J_E1C + p.mu_E2 * J_E2C;               % J/d, total mobilisation power
...
% maturation
dE_H = (1 - p.kap) * p_C - kT_J * E_H;                % J/d, maturation if juvenile
if E_H >= p.E_Hp && dE_H >= 0 % adult 
  dE_H = 0;                                        % J/d, no maturation if adult
elseif dE_H < 0
  dE_H = - k1T_J * (E_H - (1 - p.kap) * p_C/ kT_J);    % J/d, rejuvenation
end
dmax_E_H = max(0, dE_H);                           % J/d, max value of maturity

% reproduction in adults
J_E1R = J_E1C * (1 - p.kap - kT_J * E_H/ p_C);        % mol/d, allocation to reprod from res 1
J_E2R = J_E2C * (1 - p.kap - kT_J * E_H/ p_C);        % mol/d, allocation to reprod from res 2
dcM_E1R = p.kap_R1 * J_E1R * (E_H >= p.E_Hp);          % mol/d, accumulation in reprod buffer
dcM_E2R = p.kap_R2 * J_E2R * (E_H >= p.E_Hp);          % mol/d, accumulation in reprod buffer

% survival due to aging, shrinking, rejuvenation
k_C = (j_E1C - j_E1P)/ m_E1 + (j_E2C - j_E2P)/ m_E2; % 1/d, summed [p_C]/[E_m]
L_m = p.kap * min(J_E1Am/ J_E1S, J_E2Am/ J_E2S);     % cm, max structural length
dq = (q * (L/ L_m)^3 * p.s_G + p.h_a) * k_C - r * q;   % 1/d^3, change in acceleration
