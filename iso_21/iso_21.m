...
%   E_Hb      = p(14); % J, maturity threshold at birth
  ...
  kap_G = p.mu_V/ (p.mu_E1 * p.y_VE1 + p.mu_E2 * p.y_VE2); % -, growth efficiency
  % this is wrong and should be updated, see comment on section 5.2.7
  E_G = p.MV * p.mu_V/ kap_G;            % J/cm^3, [E_G] spec cost for structure
  V_b = p.E_Hb * p.kap/ E_G/ (1 - p.kap);  % cm, initial guess for length at birth
...
kap_G = p.mu_V/ (p.mu_E1 * p.y_VE1 + p.mu_E2 * p.y_VE2); % -, growth efficiency. This is wrong
E_G = p.MV * p.mu_V/ kap_G;              % J/cm^3, [E_G] spec cost for structure
V_b = p.E_Hb * p.kap/ E_G/ (1 - p.kap);    % cm, initial guess for length at birth
a_b = V_b^(1/3) * 3/ p.v;              % d, initial gues for age at birth
a_0 = a_b/ 100;                      % d, initial age from where integration starts
L_0 = a_0 * p.v/ 3;                    % cm, initial structural length
M_V0 = p.MV * L_0^3;                   % mol, initial structural mass
E_H0 = M_V0 * (p.mu_E1/ p.y_VE1 + p.mu_E2/ p.y_VE2) * (1 - p.kap)/ p.kap;  % J, initial maturity
m_E10 = M_E10/ M_V0 - 1/ p.kap/ p.y_VE1; % mol/mol, initial reserve density 1
m_E20 = M_E20/ M_V0 - 1/ p.kap/ p.y_VE2; % mol/mol, initial reserve density 2

% initialise v_H in gr_iso_21, by call with v_B_0 = []; gr_iso_21 continuates
j_E2M = p.j_E1M * p.mu_E1/ p.mu_E2;        % mol/d.mol, spec som maint for 2
[dL, j_E1_M, j_E2_M, j_E1C, j_E2C, info] = gr_iso_21(L_0, m_E10, m_E20, p.j_E1M, j_E2M, p.y_VE1, p.y_VE2, p.v, p.kap, p.rho1, []); % cm/d, change in L, ..
if info == 0
...
[E_H LEa] = ode45(@diso_21,[E_H0; p.E_Hb], [L_0; m_E10; m_E20; a_0], [], p);
