  ...
  switch model
    case {'std','stf'}
      par = {E_Hp, E_Hb, V_X, h_D, h_J, h_B0b, h_Bbp, h_Bpi, h_a, s_G, thin,  ...
         L_m, E_m, k_J, k_JX, v, g, p_M, p_Am, J_X_Am, K, kap, kap_G, E_0, aT_b, ome};
      txtPar = {'E_Hp, J', 'E_Hb, J', 'V_X, L', 'h_D, 1/d', 'h_J, 1/d', 'h_B0b, 1/d', 'h_Bbp, 1/d', 'h_Bpi, 1/d', 'h_a, 1/d', 's_G, -', 'thin, -', ...
          ...
          '[p_M] J/d.cm^3', '{p_Am}, J/d.cm^2', '{J_X_Am}, mol/d.cm^2', 'K, mol/L', 'kap, -', 'kap_G, -', 'E_0, J', 'a_b, d', 'ome, -'};
    case 'stx'
...
%% ebtmod.h: header file 
 ...
 %% ebtmod.cvf: control variable file 
  ...
  fprintf(oid, '"%s" %5.3e\n',  numPar.txt.relTol_n, numPar.relTol_n); 
  ...
  fprintf(oid, '"%s" %5.3e\n',  numPar.txt.absTol_n, numPar.absTol_n); 
...
%% ebtmod.out: read output variable file 
  ...
  fclose(out);
  n = length(data);
  tXNW = wrap(data, n/4, 4); % output (n,4)-array
