...
%%
function par = get_pars_6a(data, fixed_par, chem_par)
  % created 2015/01/19 by Bas Kooijman
  
  %% Syntax
  % par = <../get_pars_6a.m *get_pars_6a*> (data, fixed_par, chem_par)
  ...
  %
  % * fixed_par: optional 4 vector with p_M, k_J, kap_R, kap_G
  ...
  %  assumptions:
  %  abundant food (f = 1)
  %  a_b and R_m are temp-corrected to T_ref = 293 K
  ...
  if exist('fixed_par','var') == 0
     ...
     p_M  = fixed_par(1);  % J/d.cm^3, [p_M], vol-specific somatic maintenance
...
g = fzero(@(g) a_b - get_tb(g, 1, l_b)/ k_M, 3 * l_b/ a_b/ k_M);
