%% sgr_abp
% Gets specific population growth rate for the abp model

%%
function [r, info] = sgr_abp (par, T_pop, f_pop)
  % created 2019/07/30 by Bas Kooijman
  
  %% Syntax
  % [r, info] = <../sgr_abp.m *sgr_abp*> (par, T_pop, f_pop)
  
  %% Description
  % Specific population growth rate for the abp model.
  % Hazard includes 
  %
  ...
  % Buffer handling rule: continuous reproduction is used in the abp model.
  ...
  %
  %% Remarks
  % See <ssd_abp.html *ssd_abp*> for mean age, length, squared length, cubed length and other statistics.
  ...
  L_b = L_m * l_b; L_p = L_m * l_p;  s_M = l_j/ l_b; % cm, struc length at birth, puberty, ultimate
  S_b = exp(-aT_b * h_B0b);          % -, survivor prob at birth
  rT_j = kT_M * rho_j;  % 1/d, expo, von Bert growth rate
...
function dqhSC = dget_qhSC(t, qhSC, sgr, f, kap, kap_R, k_M, v, g, k, u_E0, L_b, L_p, t_p, r_j, v_Hp, s_G, h_a, h_Bbp, h_Bpi, thinning)
  ...
  l_p = L_p/ L_m; 
  R = (t > t_p) * kap_R * k_M * (s_M * l_p^2 - k * v_Hp) * (1 - kap)/ u_E0; % 1/d, reprod rate
  % use kappa-rule
