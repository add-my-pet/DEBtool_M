%% tL123_hep(par, t0, tj)
% Calculates \int_t0^tm L^i(t) dt for i=1,2,3 for the hep DEB model

%%
function [tL1, tL2, tL3] = tL123_hep(par, t0, tj)
  % created 2026/05/16 by Bas Kooijman
  
  %% Syntax
  % [tL1, tL2, tL3] = <../tL123_hep.m *tL123_hep*> (par, t0, tj)

  %% Description
  % Calculates \int_t0^tj L^i(t) dt for i=1,2,3 for the hep DEB model
  %
  % Input:
  %
  ...
  % * tj: scalar with time since start at end
  %
  % Output:
  %
  ...
  %% Example of use
  % [tL1 tL2, tL3] = tL123_hep([0.1 0.3 10 0.01 0.02 4], 0, 1000)
