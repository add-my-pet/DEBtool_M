...
function stat = ssd_std(stat, code, par, T_pop, f_pop, sgr)
  ...
  % If isnan(par), all fields are filled with NaN.
  % Hazard includes 
  %
  ...
  % * code: character string with f (0,f,1), thinning (0,1), gender(f,m), e.g. '10f'
  ...
  %
  % Output: res: structure with fields
  %
  ...
  % get output fields
  fldf = 'f0fff1'; fldf= fldf([-1 0] + 2 * strfind('0f1',code(1)));
  fldt = 'thin0thin1'; fldt= fldt([-4 -3 -2 -1 0] + 5 * strfind('01',code(2)));
  fldg = code(3);
  
  if ~isstruct(par) || isnan(sgr)
    stat = setNaN(stat, fldf, fldt, fldg);
    return
  end
  ...
  stat.(fldf).(fldt).(fldg).S_p = qhSL_a(1,3); % -, survival prob at puberty
  
  stat.(fldf).(fldt).(fldg).tS = [0, 1; t + aT_b, qhSL(:,3)];                  % d,-, convert time since birth to age for survivor probability
  stat.(fldf).(fldt).(fldg).tSs = [0 1; t + aT_b, 1 - qhSL(:,4)/ qhSL_a(2,4)]; % d,-, convert time since birth to age for survivor function of the stable age distribution
