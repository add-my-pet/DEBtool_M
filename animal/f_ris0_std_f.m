...
%%
function [f, S_b, S_p, a_b, t_p, info] = f_ris0_std_f (par)
  % created 2019/07/06 by Bas Kooijman
  
  %% Syntax
  % [f, S_b, S_p, a_b, t_p, info] = <../f_ris0_std_f.m *f_ris0_std*> (par)
  
  %% Description
  % Obtains the scaled function response at which specific population growth rate for the std model equals zero, 
  %   by solving the characteristic equation directly in f, with r = 0. We work at T_ref here.
  %
  % Input
  %
  ...
  % Similar to f_ris0_std, but f_ris0_std_f has a smaller domain of attraction
  %
  %% Example of use
