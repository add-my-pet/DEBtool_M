%% get_pars_8_stx
% Obtains 8 DEB parameters from 8 data points at abundant food for the stx model

%%
function par = get_pars_8_stx(data, t_0, p_T, fixed_par, chem_par)
...
l_T = fzero(@(l_T) get_lT(l_T, W_b, W_p, W_m, t_p, a_b, t_0, k_J, R_m, kap_R, E_G, w_E, d_V, mu_E, p_T), 0);

%D1
l_b = (W_b/ W_m)^(1/3) * (1 - l_T);
l_p = (W_p/ W_m)^(1/3) * (1 - l_T); 

%D3/D4/D5
r_B = 1/t_p * log((1 - l_T - l_b)/ (1 - l_T - l_p));
g = l_b/ (a_b - t_0)/ r_B - 1;
k_M = 3 * (1 + g)/ g * r_B;
p_M = k_M * E_G;
...
a0 = - (b2 + b3 * linf) * linf^2/ k;
a1 = ld * (2 * b2 + 3 * b3 * linf) * linf/ (k - rB);
a2 = - ld^2 * (b2 + 3 * b3 * linf)/ (k - 2*rB);
a3 = b3 * ld^3 / (k - 3*rB);
...
%D11/12/13
L_m = (W_m/ (1 + w))^(1/3)/ (1 - l_T); % cm, maximum structural length
...
function var = get_lT(l_T, W_b, W_p, W_m, t_p, a_b, t_0, k_J, R_m, kap_R, E_G, w_E, d_V, mu_E, p_T)
...
L_T = p_T/ p_M;              % cm, heating length
lT = L_T/ L_m;              % -, scaled heating length
