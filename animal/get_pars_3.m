%% get_pars_3
% Obtains 3 DEB parameters from 3 data points at abundant food

%%
function par = get_pars_3(data, fixed_par, chem_par)
  % created 2015/01/18 by Bas Kooijman
  
  %% Syntax
  % par = <../get_pars_3.m *get_pars_3*> (data, fixed_par, chem_par)
  ...
  % Input
  %
  % * data: 3-vector with zero-variate data
  %
  ...
  % Output
  %
  % * par: 3-vector with DEB parameters
  %
  ...
  W_m = data(3); % g, maximum wet weight
