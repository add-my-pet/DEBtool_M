...
%%
function [filter flag] = filter_pars_8 (par, fixed_par, chem_par)
  % created 2015/01/19 by Bas Kooijman
  
  %% Syntax
  % [filter flag] = <../filter_pars_8.m *filter_pars_8*> (par, fixed_par, chem_par)
  ...
  %   9: numerical problems in get_tp
  
  %% Remarks
  % The theory behind iget_pars_8 is discussed in 
  %   <http://www.bio.vu.nl/thb/research/bib/LikaAugu2014.html LikaAugu2014>.
  % Meant to run prior to <iget_pars_8.html *iget_pars_8*>.
  % With acceleration, see <filter_par_9.html *filter_par_9*>.

  %% Example of use
  % See <../mydata_get_pars_8.m *mydata_get_pars_8*>
  
  % assumptions: no acceleration
  % abundant food (f=1)
  ...
  [t_p t_b l_p l_b  info] = get_tp(pars_tp, 1, l_b);
  if info ~= 1
    filter = 0;
    flag = 9;
    return
