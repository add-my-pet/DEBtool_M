%% iget_pars_8_stx
% Obtains 8 data points from 8 DEB parameters at abundant food for the stx model

%%
function data = iget_pars_8_stx(par, t_0, p_T, fixed_par, chem_par)
  ...
  l_p = get_lp1 ([g, k, l_T, v_Hb, v_Hp, 1], 1, l_b); 
  r_B = k_M/ 3 * g/ (1 + g);
  t_p = log((1 - l_T - l_b)/ (1 - l_T - l_p))/ r_B;  a_p = a_b + t_p;
%   t_p = log((1 - l_b)/ (1 - l_p))/ r_B;  a_p = a_b + t_p;

  Ww_m = (L_m * (1 - l_T))^3 * (1 + w); % g, ultimate wet weight
  Ww_b = l_b^3 * L_m^3 * (1 + w);
  Ww_p = l_p^3 * L_m^3 * (1 + w);
