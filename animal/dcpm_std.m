...
%%
function dXvars = dcpm_std(t, Xvars, tTC, tJX, V_X, h_D, h_J, q_b, h_Ab, h_B0b, h_Bbp, h_Bpi, h_a, s_G, thin, S_b, aT_b, L_b, L_m, E_Hb, E_Hp, E_m, k_J, k_JX, v, g, p_M, p_Am, J_X_Am, K, kap, kap_G, del_X)
% created 2020/03/09 by Bob Kooi & Bas Kooijman
  
%% Syntax
% dXvars = <../dcpm_std.m *dcpm_std*> (t, Xvars, par, tTC, tX)
...
% * tX: (nX,2)-array with time and food density in supply 
%
% Output:
%
...
%% Remarks
% aT_b < t_R should apply; changes in embryo states are ignored, and embryo states are set at birth values 
    
  [X, q, h_A, L, L_max, E, E_R, E_H, N] = cpm_unpack(Xvars); 
  e = E/ E_m; L2 = L .* L; L3 = L .* L2; 
  
  if t < aT_b % set embryos at birth value, since changes are too fast, below the embryo changes are set to 0
  ...
  p_C = p_A - L3 .* (dE + E .* r); % J/d, reserve mobilisation rate
  ...
  dE_R = p_R .* (E_H > E_Hp) + 0.000*min(0, kap * p_C - p_M * L3) .* (E_R > 0);
  ...
  if t < aT_b % set changes of embryo states to zero
