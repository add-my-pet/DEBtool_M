...
%%
function [S_b, q_b, h_Ab, tau_b, l_b, u_E0, info] = get_Sb_foetus(p, f)
  ...
  elvqhS_0 = [f; 1e-9; 0; 0; 0; 1]; % initial value
  options = odeset('NonNegative',ones(4,1), 'AbsTol',1e-9, 'RelTol',1e-9);  
  [tau, elvqhS] = ode45(@dget_elvqhS, [0 tau_b], elvqhS_0, options, p);
  q_b = max(0,elvqhS(end,4)); h_Ab = max(0,elvqhS(end,5)); S_b = max(0,elvqhS(end,6)) * exp(- p(6) * tau_b);
