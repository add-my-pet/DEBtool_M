...
%%
function data = iget_pars_9_new(par, fixed_par, chem_par)
  ...
  sM = fzero(@fnget_sM, (v_Hj/ v_Hb)^(1/3), [], 1, l_b, g, k, 0, v_Hb, v_Hj);
  l_j = sM * l_b;
  
  l_p = get_lp1 ([g, k, 0, v_Hj, v_Hp, sM], 1, l_j);
  
  rj = g * (1/ l_b - 1)/ (1 + g); % scaled exponential growth rate between b and j
  t_j = t_b + (log(sM)) * 3/ rj;     % scaled age at metamorphosis

  rB = 1/ 3/ (1 + 1/ g);             % scaled von Bert growth rate between j and i
  l_i = sM;                           % scaled ultimate length
  t_p = t_j + log((l_i - l_j)/ (l_i - l_p))/ rB;
...
% a_j = t_j/ k_M  % d, age at metamorphosis
  ...
  R_m = k_M * kap_R * (1 - kap) * (sM^3 - k * v_Hp) / uE0; % #/d, maximum reprod rate
