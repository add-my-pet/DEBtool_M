...
%%
function stat = ssd_abp(stat, code, par, T_pop, f_pop, sgr)
  % created 2019/07/30 by Bas Kooijman
  
  %% Syntax
  % stat = <../ssd_abp.m *ssd_abp*> (stat, code, par, T_pop, f_pop, sgr)
  ...
  L_b = L_m * l_b; L_p = L_m * l_p;  % cm, structural length at birth, puberty = ultimate
  ...
  p_A = L_p^2 * pT_Am * s_M * f; % J/d, assimilation = mobilisation of adults
  p_R = max(0, (1 - kap) * p_A - kT_J * E_Hp); % J/d, mean allocation to reproduction of adults
...
function dqhSL = dget_qhSL(t, qhSL, t_p, sgr, f, kap, kap_R, k_M, v, g, k, u_E0, L_b, L_p, L_m, r_j, v_Hp, s_G, h_a, h_Bbp, h_Bpi, thinning)
