...
%%
function [stat, txtStat] = ssd_asj(stat, code, par, T_pop, f_pop, sgr)
  % created 2019/07/30 by Bas Kooijman, modified 2020/02/19
  
  %% Syntax
  % [stat, txtStat] = <../ssd_asj.m *ssd_asj*> (stat, code, par, T_pop, f_pop, sgr)
  ...
  %
  %   - a_b: d, age at birth
  %   - t_s: d, time since birth at start accel
  ...
  %   - S_s: -, survival probability at start accele
  %   - S_j: -, survival probability at end accel
  ...
  %   - theta_bs: -, fraction of individuals that is between b and s
  %   - theta_sj: -, fraction of individuals that is between s and j
  %   - theta_jp: -, fraction of individuals that is between l and p
  ...
  tT_s = (tau_s - tau_b)/ kT_M; % d, time since birth at start accel
  stat.(fldf).(fldt).(fldg).t_s = tT_s; txtStat.units.t_s  = 'd';  txtStat.label.t_s = 'time since birth at start accel';
  tT_j = (tau_j - tau_b)/ kT_M; % d, time since birth at metam
  stat.(fldf).(fldt).(fldg).t_j = tT_j; txtStat.units.t_j  = 'd';  txtStat.label.t_j = 'time since birth at metam';
  ...
  L_b = L_m * l_b; L_s = L_m * l_s; L_j = L_m * l_j; L_p = L_m * l_p; L_i = L_m * l_i; % cm, structural length at birth, ultimate
  ...
  pars_qhSL = {aT_b, tT_s, tT_j, tT_p, sgr, f, vT, L_b, L_s, L_j, L_i, L_m, rT_j, rT_B, s_G, hT_a, h_Bbs, h_Bsj, h_Bjp, h_Bpi, thinning};
  ...
  S_s = qhSL_event(1,3);  % -, survival prob at start accel
  stat.(fldf).(fldt).(fldg).S_s = S_s; txtStat.units.S_s  = '-';  txtStat.label.S_s = 'survival probability at start accel';
  S_j = qhSL_event(2,3);  % -, survival prob at metam
  stat.(fldf).(fldt).(fldg).S_j = S_j; txtStat.units.S_j  = '-';  txtStat.label.S_j = 'survival probability at metam';
  ...
  theta_bs = qhSL_event(1,4)/ (t_0b + t_bi);    % -, fraction of individuals that is between b and s
  stat.(fldf).(fldt).(fldg).theta_bs = theta_bs; txtStat.units.theta_bs = '-'; txtStat.label.theta_bs = 'frac of ind that is between b and s';
  theta_sj = qhSL_event(2,4)/ (t_0b + t_bi) - theta_bs; % -, fraction of individuals that is between s and j
  stat.(fldf).(fldt).(fldg).theta_sj = theta_sj; txtStat.units.theta_sj = '-'; txtStat.label.theta_sj = 'frac of ind that is between s and j';
  theta_jp = qhSL_event(3,4)/ (t_0b + t_bi) - theta_bs - theta_sj; % -, fraction of individuals that is between j and p
  stat.(fldf).(fldt).(fldg).theta_jp = theta_jp; txtStat.units.theta_jp = '-'; txtStat.label.theta_jp = 'frac of ind that is between j and p';
  ...
  del_an = theta_pi/ (theta_bs + theta_sj + theta_pi); % fraction of post-natals that is adult
  ...
  t_pi = t_bi - qhSL_event(3,4); % d, \int_{a_p}^\infty S(t)*exp(-sgr*t) dt
  L_pi = (qhSL(end,5)-qhSL_event(3,5))/ t_pi; L2_pi = (qhSL(end,6)-qhSL_event(3,6))/ t_pi; L3_pi = (qhSL(end,7)-qhSL_event(3,7))/ t_pi; % mean L^1,2,3 for adults
  ...
  %
  sL2_pi = (qhSL(end,10)-qhSL_event(3,10))/ t_pi; % cm^2, \frac{\int_{a_p}^\infty s_M(t) L^2*S(t)*exp(-sgr*t) dt} {\int_{a_p}^\infty S(t)*exp(-sgr*t) dt}
...
function dqhSL = dget_qhSL(t, qhSL, a_b, t_s, t_j, t_p, sgr, f, v, L_b, L_s, L_j, L_i, L_m, r_j, r_B, s_G, h_a, h_Bbs, h_Bsj, h_Bjp, h_Bpi, thinning)
...
function stat = setNaN(stat, fldf, fldt, fldg)
    ...
    stat.(fldf).(fldt).(fldg).S_b = NaN;      stat.(fldf).(fldt).(fldg).S_s = NaN;    stat.(fldf).(fldt).(fldg).S_j = NaN;  stat.(fldf).(fldt).(fldg).S_p = NaN; 
    ...
    stat.(fldf).(fldt).(fldg).theta_0b = NaN; stat.(fldf).(fldt).(fldg).theta_bs = NaN; 
    stat.(fldf).(fldt).(fldg).theta_sj = NaN; stat.(fldf).(fldt).(fldg).theta_jp = NaN; stat.(fldf).(fldt).(fldg).theta_pi = NaN;   
