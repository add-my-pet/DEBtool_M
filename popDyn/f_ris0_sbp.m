%% f_ris0_sbp
% Gets scaled functional response at with the specific population growth rate is zero for the sbp model

%%
function [f, info] = f_ris0_sbp (par)
  ...
  % Output
  %
  % * f: scaled func response at which r = 0 for the sbp model
  ...
  if charEq0(f_0, pars_charEq0{:}) > 0
    fprintf(['Warning from f_ris0_sbp: f for which r = 0 is very close to that for R_i = 0 and thinning = ', num2str(thinning), '\n']);
    f = f_0; info = 1; return
  end
  ...
  if charEq0(1, pars_charEq0{:}) < 0
    fprintf(['Warning from f_ris0_sbp: no f detected for which r = 0 and thinning = ', num2str(thinning), '\n']);
    info = 0; f = f_0; return
  end
  ...
  if i == 18 
    info = 0;
    fprintf('f_ris0_sbp warning: no convergence for f in 18 steps\n')
  elseif f_1 - f_0 > 1e-4
    info = 0;
    fprintf('f_ris0_sbp warning: interval for f < 1e-4, norm = %g\n', norm)
  else
    info = 1;
  end
