...
globals[
  ...
  ; h_thin   ; true/false, hazard for thinning. If true it changes in time for each turtle
...
to setup
  ...
  file-open "tJX.txt"    ; knots with time, food input into the reactor
  ...
  file-open "tTC.txt"    ; knots with time, temperature correction factor
  while [file-at-end? = false] [
    let row (list) ; empty list
    set row insert-item 0 row file-read ; t
...
to go
  ...
  let eaten 0 ; mol/d, initiate food disappearence rate
  ...
  ; notice that X is a density, while eaten is an absolute flux; in the EBT method population numbers are densities
  ...
  ask turtles with [E_H > E_Hb] [
    ...
    if E_H > E_Hpi [set E_H E_Hpi] ; J, do not allow maturity to exceed puberty level
    ...
    ifelse h_thin = 1 [set h_thini r * 2 / 3] [set h_thini 0] ; 1/d, hazard rate due to thinning
  ...
  ask turtles with [(E_H > E_Hb) and (E_H < E_Hpi)] [if (h_Bbp + h_thini + h_age) / tickRate > random-float 1 [ die ]]
  ask turtles with [E_H = E_Hpi] [if (h_Bpi + h_thini + h_age) / tickRate > random-float 1 [ die ]]
...
The temperature is specified via a spline with knots tTC, with time and temperature correction factors.
...
The reactor is well-stirred, so homogeneous in terms of food and population density.
For this reason this model does not work with patches.
...
Thinning never applies to embryos; it exactly compensates the increase of total food intake by a cohort due to growth, by as reduction in numbers. 
...
Since the pre-natal states, reserve and structure, change much faster than post-natal, they are set at birth-values, till age passes age-at-birth.
...
Rejuvenation, due to failing to pay maturity maintenance costs, affects survival.
...
This Matlab function writes the NetLogo model and sets the parameter values, using the AmP collection.
...
The model can also be run directly under NetLogo and its gui (simply load std.nlogo in NetLogo, but make sure that files tJX.txt, tTC.txt and eaLE.txt exist in the same directory).
...
Moreover, if the parameter that you want to change occurs in set_pars.txt, the setup-procedure will overwrite it. You might remove the set_pars file to avoid this.
...
Any edits in code within NetLogo leads automatically to an overwrite of the stored model-definition, adding a large section with code for shape-definitions to move shapes across the screen, even if these shapes are not used.
