...
%%
function [stat, txtStat] = ssd_abj(stat, code, par, T_pop, f_pop, sgr)
  % created 2019/07/28 by Bas Kooijman, modified 2020/02/19
  
  %% Syntax
  % [stat, txtStat] = <../ssd_abj.m *ssd_abj*> (stat, code, par, T_pop, f_pop, sgr)
  
  %% Description
  % Mean L, L^2, L^3, Ww, given f and r, on the assumption that the population has the stable age distribution.
  % Use sgr_abj to obtain sgr. Background hazards are not standard in par as produced by AmP; add them before use.
  ...
  %   - S_j: -, survival probability at metam
  ...
  %   - theta_bj: -, fraction of individuals that is pre-metam juvenile
  %   - theta_jp: -, fraction of individuals that is post-metam juvenile
  ...
  pars_qhSL = {aT_b, tT_j, tT_p, sgr, f, vT, L_b, L_j, L_i, L_m, rT_j, rT_B, s_G, hT_a, h_Bbj, h_Bjp, h_Bpi, thinning};
  ...
  S_j = qhSL_event(1,3);  % -, survival prob at metam
  ...
  theta_bj = qhSL_event(1,4)/ (t_0b + t_bi);      % -, fraction of individuals that is pre-metam juvenile
  stat.(fldf).(fldt).(fldg).theta_bj = theta_bj; txtStat.units.theta_bj = '-'; txtStat.label.theta_bj = 'frac of ind that is pre-metam juvenile';
  theta_jp = qhSL_event(2,4)/ (t_0b + t_bi) - theta_bj; % -, fraction of individuals that is post-metam juvenile
  stat.(fldf).(fldt).(fldg).theta_jp = theta_jp; txtStat.units.theta_jp = '-'; txtStat.label.theta_jp = 'frac of ind that is post-metam juvenile';
  theta_pi = 1 - theta_0b - theta_bj - theta_jp;  % -, fraction of individuals that is adult
  stat.(fldf).(fldt).(fldg).theta_pi = theta_pi; txtStat.units.theta_pi = '-'; txtStat.label.theta_pi = 'frac of ind that is adult';
  del_an = theta_pi/ (theta_bj + theta_jp + theta_pi); % fraction of post-natals that is adult
  ...
  %
  sL2_pi = (qhSL(end,10)-qhSL_event(2,10))/ t_pi; % cm^2, \frac{\int_{a_p}^\infty s_M(t) L^2*S(t)*exp(-sgr*t) dt} {\int_{a_p}^\infty S(t)*exp(-sgr*t) dt}
  ...
  % 
  n_O = nO_d2w(n_O, [d_X, d_V, d_E, d_P]); % -, chemical indices of organics  on wet food: (in cols)  X V V_dead E E_dead P
  Y_M = -(n_M\n_O) * [-1; Y_VX+Y_VX_dead; Y_EX+Y_EX_dead; Y_PX];     % mol/mol, yields od minerals on wet food
...
% event dead_for_sure
function [value,isterminal,direction] = dead_for_sure(t, qhSL, t_j, t_p, varargin)
  value = [t - t_j; t - t_p; qhSL(3) - 1e-6];  % trigger 
  isterminal = [0 0 1];                        % terminate after last event
  direction  = [];                             % get all the zeros
end

function dqhSL = dget_qhSL(t, qhSL, a_b, t_j, t_p, sgr, f, v, L_b, L_j, L_i, L_m, r_j, r_B, s_G, h_a, h_Bbj, h_Bjp, h_Bpi, thinning)
...
function stat = setNaN(stat, fldf, fldt, fldg)
    ...
    stat.(fldf).(fldt).(fldg).S_b = NaN;      stat.(fldf).(fldt).(fldg).S_j = NaN;  stat.(fldf).(fldt).(fldg).S_p = NaN; 
    stat.(fldf).(fldt).(fldg).a_b = NaN;      stat.(fldf).(fldt).(fldg).t_j = NaN;  stat.(fldf).(fldt).(fldg).t_p = NaN;   
    stat.(fldf).(fldt).(fldg).tS = [NaN NaN]; stat.(fldf).(fldt).(fldg).tSs = [NaN NaN];
    stat.(fldf).(fldt).(fldg).R = NaN;        stat.(fldf).(fldt).(fldg).J_X = NaN; 
    stat.(fldf).(fldt).(fldg).theta_0b = NaN; stat.(fldf).(fldt).(fldg).theta_bj = NaN; stat.(fldf).(fldt).(fldg).theta_jp = NaN; stat.(fldf).(fldt).(fldg).theta_pi = NaN;   
