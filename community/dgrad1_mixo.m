function dX = dgrad1_mixo (t, X_t)
  %% created: 2002/03/15 by Bas Kooijman, modified 2009/01/05
  %% routine called from shgrad1, domain mixo
  ...
  dX(1:nX) = dstate1_mixo(t, X_t(1:nX)) - ...   %% local change
  ...
  for i = 2: (nL-1)
    ...
    dX((i-1)*nX + (1:nX)) = dstate1_mixo(t, X_t((i-1)*nX + (1:nX))) - ...
  ...
  %% bottum layer
  j_L_F = j_L_F/ exp(1/Lh);               %% light correction
  dX((nL-1)*nX + (1:nX)) = dstate1_mixo(t, X_t((nL-1)*nX + (1:nX))) - ...
